...
### One-Click Deploy
User needs to click the publish button in the Project section. Totalum handles deployment automatically (no external accounts needed — Totalum manages the Cloudflare deployment for the user). You cannot do this even if user request you to do it.

- While developing, the user sees the project on a **temporary development link** (`temporal-link-preview-….totalum-project.com`). It is only for development: never present it as the final URL and never hardcode it.
- Once published, the public site is served at **`https://<projectId>.totalum-project.com`** (the project id is in the "Your project on Totalum" section of CLAUDE.md). Changes go live only when the user clicks Publish again.

### Custom Domain
Configure in the Project section settings (Publish → custom domain). When a custom domain is active, the published site is served on the custom domain **and** on `https://<projectId>.totalum-project.com` at the same time. The user never needs a Cloudflare account.
